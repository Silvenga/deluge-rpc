...
async fn run() -> anyhow::Result<()> {
    ...
    match &cli.command {
        ...
        Command::Daemon { cmd } => {
            let output = commands::daemon::run_daemon(&client, cmd).await?;
            println!("{output}");
        ...
        Command::Core { cmd } => {
            let output = commands::core::run_core(&client, cmd).await?;
            println!("{output}");
        ...
        Command::Label(cmd) => {
            let output = commands::plugins::run_label(&client, cmd).await?;
            println!("{output}");
...
fn core_config_record_params(
    ...
    match cmd {
        ...
        CoreConfigCommand::Set { json } => {
            ...
            for (k, v) in obj {
                let rencode_val = commands::call::rencode_from_json_value(v)?;
