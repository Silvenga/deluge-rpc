...
- 2 methods not available on the test daemon: `core.get_ssl_listen_port` (method not registered), `label.get_labels` (
  Label plugin not enabled).
- The mock server auto-serves `daemon.login` for any credentials — cassettes never contain login interactions or
  passwords.
